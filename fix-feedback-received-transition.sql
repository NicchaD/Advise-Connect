-- TEMPORARY FIX: Run this SQL directly in your Supabase SQL Editor
-- This fixes the "Feedback Received" to "Implemented" transition issue

-- First, let's check if the transition exists in the database
SELECT * FROM status_transitions WHERE from_status = 'Feedback Received' AND to_status = 'Implemented';

-- If the above query returns no results, insert the missing transition
INSERT INTO status_transitions (from_status, to_status, role_required) 
VALUES ('Feedback Received', 'Implemented', 'Advisory Consultant')
ON CONFLICT (from_status, to_status) DO NOTHING;

-- Update the function to handle "Feedback Received" to "Implemented" transition explicitly
...
BEGIN
  ...
  -- Special case: Feedback Received to Implemented - keep current assignee (original assignee)
  ELSIF current_status = 'Feedback Received' AND new_status = 'Implemented' THEN
    new_assignee_id := current_assignee_id;
  ...
  -- Special case: Review to Approved - assign back to original assignee
  ELSIF current_status = 'Review' AND new_status = 'Approved' THEN
    new_assignee_id := original_assignee_id;
  -- NEW: Review to Approval - assign back to original assignee
  ELSIF current_status = 'Review' AND new_status = 'Approval' THEN
    new_assignee_id := original_assignee_id;
  -- Special case: Approved to Implementing - keep original assignee (no change)
  ELSIF current_status = 'Approved' AND new_status = 'Implementing' THEN
    new_assignee_id := original_assignee_id;
  -- Special case: Approval to Implementing - keep current assignee (no change) and capture implementation start date
  ELSIF current_status = 'Approval' AND new_status = 'Implementing' THEN
    new_assignee_id := current_assignee_id;
  -- Special case: Approval to Estimation or Review - assign back to original assignee (legacy support)
  ELSIF current_status = 'Approval' AND new_status IN ('Estimation', 'Review') THEN
    new_assignee_id := original_assignee_id;
  ELSE
  ...
  -- Update the request
  IF current_status = 'Approval' AND new_status = 'Implementing' THEN
    -- Capture implementation start date when transitioning from Approval to Implementing
    UPDATE requests 
    SET 
      status = new_status,
      assignee_id = new_assignee_id,
      implementation_start_date = now(),
      updated_at = now()
    WHERE id = p_request_id;
  ELSE
    -- Normal status update without implementation start date
    UPDATE requests 
    SET 
      status = new_status,
      assignee_id = new_assignee_id,
