-- Drop the existing problematic functions with CASCADE to remove all dependencies
DROP FUNCTION IF EXISTS sync_profiles_from_advisory_team() CASCADE;
DROP FUNCTION IF EXISTS sync_advisory_team_members() CASCADE;

-- Create improved sync functions that prevent infinite recursion
...
CREATE TRIGGER sync_advisory_team_members_trigger
