-- Fix automatic reassignment by using the existing get_next_assignee function
...
BEGIN
  ...
  INTO current_status, current_assignee_id, original_assignee_id, request_advisory_services
  FROM requests r
  ...
  ELSE
    -- Use the existing get_next_assignee function for proper load balancing
  ...
  IF current_assignee_id IS DISTINCT FROM new_assignee_id THEN
    ...
    -- Add new assignment with required status_at_assignment field
