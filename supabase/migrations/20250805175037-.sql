-- Fix the logic for reverting assignee when transitioning from Review back to Estimation
...
BEGIN
  ...
  IF new_status = 'Estimation' AND old_status = 'Review' THEN
    ...
    ELSE
      -- Look for the first assignee change in history to find who was originally assigned
      SELECT old_value::uuid INTO original_assignee_id
      FROM request_history 
      WHERE request_id = p_request_id 
        AND action LIKE '%Assignee%' 
        AND old_value IS NOT NULL
        AND old_value != 'Unassigned'
      ORDER BY performed_at ASC
      LIMIT 1;
      
      -- If no assignee changes found, look for who was assigned when status first changed from New
      IF original_assignee_id IS NULL THEN
        -- Get the assignee who was assigned when request was initially processed
        -- Look for the assignee before any status changes happened
        SELECT DISTINCT atm.user_id INTO original_assignee_id
        ...
        WHERE atm.title = 'Advisory Consultant'
          ...
          -- Find the consultant with least workload who would have been assigned originally
        ORDER BY (
          SELECT COUNT(*) FROM requests r2 
          WHERE r2.assignee_id = atm.user_id 
          AND r2.status NOT IN ('Implemented', 'Reject', 'Closed', 'Cancelled')
          AND r2.created_at < request_row.created_at
        ) ASC
        LIMIT 1;
      END IF;
    END IF;
