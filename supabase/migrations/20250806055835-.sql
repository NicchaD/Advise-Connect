...
BEGIN
  ...
  IF new_status = 'New' AND old_status = 'Pending Review' THEN
    -- First try to find Advisory Consultant with matching advisory service and expertise, excluding original assignee
    ...
    IF NOT FOUND THEN
      -- Fallback: Find Advisory Service Head with matching advisory service and least requests
      SELECT atm.user_id, atm.name INTO consultant_row
      FROM advisory_team_members atm
      LEFT JOIN requests r ON r.assignee_id = atm.user_id AND r.status NOT IN ('Implemented', 'Reject', 'Closed', 'Cancelled')
      ...
      IF NOT FOUND THEN
        -- Still no one found, return error message
