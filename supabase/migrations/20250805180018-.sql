-- Fix the SQL error "for SELECT DISTINCT, ORDER BY expressions must appear in select list"
  ...
  IF new_status = 'Estimation' AND old_status = 'Review' THEN
    ...
    ELSE
      ...
      IF original_assignee_id IS NULL THEN
        ...
        -- Remove DISTINCT and fix ORDER BY issue
        SELECT atm.user_id INTO original_assignee_id
