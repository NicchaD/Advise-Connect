-- Update the status transition function to handle Approval to Review/Estimation transitions
...
BEGIN
  ...
  -- Handle transition from "Approval" to "Review" or "Estimation" - assign back to original assignee
  IF (new_status = 'Review' OR new_status = 'Estimation') AND old_status = 'Approval' THEN
  ...
  ELSIF new_status = 'Estimation' AND old_status = 'Review' THEN
    -- Get the original assignee from the stored field
    original_assignee_id := request_row.original_assignee_id;
  ...
  ELSIF new_status IN ('Implementing') THEN
