-- First, let's check if triggers exist and drop them to recreate
DROP TRIGGER IF EXISTS set_original_assignee_on_insert ON public.requests;
DROP TRIGGER IF EXISTS set_original_assignee_on_update ON public.requests;

-- Recreate the function to store the original assignee
