-- First, drop the existing status check constraint to allow updates
...
-- Update the database function to use "Approved" instead of "Approval"
...
BEGIN
  ...
  -- Special case: Approved to Estimation or Implementing - assign back to original assignee (CHANGED FROM 'Approval')
  ELSIF current_status = 'Approved' AND new_status IN ('Estimation', 'Implementing') THEN
    new_assignee_id := original_assignee_id;
  ELSE
...
-- Now add the new constraint with "Approved" instead of "Approval"
