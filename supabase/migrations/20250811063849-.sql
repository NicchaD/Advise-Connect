-- Fix the auto assignment trigger to handle service_specific_data properly
...
BEGIN
  ...
  IF NEW.status = 'New' AND NEW.assignee_id IS NULL THEN
    -- Ensure service_specific_data is not null and has selectedOfferings
    ...
    ELSE
      -- Fallback to empty array if no service offerings
