-- Add "Under Discussion" status to the check constraint
ALTER TABLE requests DROP CONSTRAINT IF EXISTS requests_status_check;
ALTER TABLE requests ADD CONSTRAINT requests_status_check 
CHECK (status IN ('New', 'Under Discussion', 'Estimation', 'Review', 'Approval', 'Implementing', 'Awaiting Feedback', 'Feedback Received', 'Implemented', 'Reject', 'Closed', 'Cancelled', 'Pending Review'));

-- Update status transitions to include the new "Under Discussion" status
-- Remove existing transitions from "New" to "Estimation"
DELETE FROM status_transitions WHERE from_status = 'New' AND to_status = 'Estimation';

-- Add new transitions for the modified workflow
-- From "New" can go to "Under Discussion" or "Pending Review"
INSERT INTO status_transitions (from_status, to_status, role_required) VALUES
('New', 'Under Discussion', 'Advisory Consultant'),
('New', 'Pending Review', 'Advisory Consultant');

-- From "Under Discussion" can go to "Estimation"
INSERT INTO status_transitions (from_status, to_status, role_required) VALUES
('Under Discussion', 'Estimation', 'Advisory Consultant');

-- Update the update_request_status_and_assignee function to handle new transitions
...
BEGIN
  ...
  IF new_status = 'Awaiting Feedback' AND old_status = 'Implementing' THEN
  ...
  -- Handle transitions where assignee remains the same (NEW transitions)
  ELSIF (new_status = 'Under Discussion' AND old_status = 'New') OR 
        (new_status = 'Estimation' AND old_status = 'Under Discussion') THEN
